...
class TestMemmapSeqs:
    ...
    def test_memmap_seqs_fetch(self):
        ...
        # Manually construct the internal state
        # Create sequences: [1,2,3] for id=1, [4,5] for id=2
        seq_data = np.array([1, 2, 3, 4, 5], dtype=np.int32)
        seqs._seq_arr = MemmapArray(seq_data)
        
        # Create offset array: [0, 3, 5] (start positions for each sequence)
        offsets = np.array([0, 3, 5], dtype=np.uint64)
        seqs._offset_arr = MemmapArray(offsets)
        
        # Create id to offset map: id 1 -> index 0, id 2 -> index 1
        ...
        result = seqs.fetch(999)  # Non-existent ID
